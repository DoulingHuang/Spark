...
if __name__ == "__main__":
    ...
    user_activity_df_sorted = user_activity_df.orderBy(user_activity_df["number_of_add_to_carts"].desc())

    query = user_activity_df_sorted.selectExpr("cast(visitor as string) as key",
                                               "cast(concat(visitor, ',', number_of_sales, ',',number_of_add_to_carts, ',', \
                                               number_of_page_views, ',', number_of_events) as string) as value") \
                                   .writeStream\
                                   .outputMode("complete")\
                                   .format("kafka") \
                                   .option("kafka.bootstrap.servers", "localhost:9092,localhost:9093") \
                                   .option("topic", "logs_stream_result") \
                                   .option("checkpointLocation", "hdfs://localhost/user/cloudera/structured_streaming_101/checkpoint") \
